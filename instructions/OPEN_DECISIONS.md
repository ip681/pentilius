...
- **Research branches/technology list/costs/effects** → Milestone 2 ships 4 technologies (`apps/api/prisma/seed.ts` `researchTypes`/`researchLevelCosts`: Metal Production, Crystal Production, Weapon Systems, Hull Engineering) each granting +5%/level (placeholder rate) up to level 5, applied in `EconomyService.getResearchMultiplier` (production) and `CombatService.computePlayerStats` (combat). Only one technology can be actively researching at a time (owner decision, 2026-09-11 — reversed from letting all 4 run in parallel, since with only 4 techs capped at level 5, parallel research let players exhaust the whole tree too quickly), same single-slot rule as expeditions and base building.
- **Boss Formations (owner decision, 2026-09-11 — fully replaces the earlier open-lobby "Boss Hunts" model described in prior versions of this file):** 4 bosses, one per zone/Territory (`apps/api/prisma/seed.ts`'s `bossData`: Verdant Warden, Ridgeback Alpha, Crimson Harbinger, Frost Sovereign — the middle two pre-existing, the outer two new placeholder-stat content). A formation is a squad of exactly 5 hard race-locked slots (`BossFormationSlot`, one per `Race`) built against one boss — creating one auto-fills the creator's own race slot. Visibility (`visibleToClanOnly`/`visibleToFriendsOnly`) mirrors `MarketListing`'s boolean-union semantics exactly, editable by the creator only during the first `GAME_BALANCE.bossFormations.visibilityEditWindowHours` (2h) of the 3h enrollment window (`enrollmentWindowHours`). A single rolling 24h cooldown (`Player.nextBossFormationActionAt`, `actionCooldownHours`) covers both creating and joining — not two separate limits, and deliberately personal/rolling rather than a fixed global reset hour, to stay fair across timezones. No kick, and no self-leave once joined — the frontend gates both create and join behind `ConfirmButton`. Combat is one combined party-vs-boss fight (`combat.service.ts`'s `aggregateFormationCombatStats`, built on a new `computeCombatBreakdown`/`PlayerCombatBreakdown` split of the existing per-player `computePlayerStats`): raw stats sum per-player-factor-first, then Excellent-option percentages (`INCREASE_DAMAGE`/`CRITICAL_DAMAGE`/`INCREASE_MAX_HP`/`DAMAGE_DECREASE`, capped via `maxDamageDecrease`) are summed across every filled slot and applied ONCE to the combined total — deliberately stronger than solo combat's per-player-then-summed model. `DAMAGE_REFLECT` is excluded entirely (PvP-only). No contribution-weighted rewards: every participant of a WIN gets an equal XP split (`Math.floor(xpReward / filledSlotCount)`), the same guaranteed tier-matched box (`Boss.guaranteedBoxItemDefinitionId`, on top of the pre-existing independent `BossLootDrop` chance rolls, unchanged), and the fight's full `damageDealt` credited (not divided) to every winner's `BossFormationDailyPoints` row for that UTC calendar day — a separate fixed-00:00-UTC clock from the personal action cooldown, collected now but with no ranking/reward-tier UI yet (explicitly deferred). A loss gives nothing at all. Resolution is lazy (checked on next read, no cron), same pattern as buildings/research/expeditions. `minLevel` (flat 10, not per-zone) and the new bosses' exact stats/art are explicit placeholders per the owner's own "после ще го измислим." Full design history: `project_boss_formations.md` in the auto-memory store.
